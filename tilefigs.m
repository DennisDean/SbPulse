...
xlen = fix(maxpos(3)/ncols) - 15; % new tiled figure width
ylen = fix(maxpos(4)/nrows) - 89; % new tiled figure height
...
pnum=0;
for iy = 1:nrows
  ypos = maxpos(4) - fix((iy)*maxpos(4)/nrows) + border +15; % figure location (row)
  for ix = 1:ncols
